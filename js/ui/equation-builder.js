...
const CURRENT_MODEL_LABEL = `<span class="eq-current-model-label">Model:</span>`;
...
export function buildPanelEquationHTML(spec = {}) {
  ...
  if (includeMemberFE) {
    ...
    legend.push([
      "γᵢ",
      "grower fixed effects — absorb baseline price differences between growers",
    ]);
  }
  if (includeMonthFE) {
  ...
  if (includeCostControl) {
    ...
    legend.push([
      "β",
      "input cost index (fertiliser/energy/packaging) — supply-cost control",
    ]);
  }
  ...
  if (extendCartelDummy) {
    ...
    legend.push([
      "δ",
      "overcharge — dummy wrongly extended over the run-off months",
    ]);
  } else {
  ...
  return (
    CURRENT_MODEL_LABEL +
    renderEquation(`p<sub>it</sub> = ${terms.join(" + ")}`, legend)
  );
}
...
export function buildDidEquationHTML(spec = {}) {
  ...
  if (includeTrend) {
    terms.push("φ·t");
    legend.push(["φ", "common time trend"]);
  }
  if (includeGroupTrend) {
    terms.push("ψ·(Cartelised<sub>it</sub>×t)");
    legend.push(["ψ", "UK-specific trend"]);
  }
